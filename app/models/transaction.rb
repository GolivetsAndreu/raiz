class Transaction < ApplicationRecord
  ...
  validate :teem_can_do_teem_user, if: -> { from_account_id.present? && to_account_id.present? }
  ...
  def teem_can_do_teem_user
    if from_account.objectable_type == 'Teem'
      errors.add(:to_account_id, "can't be get money from this account") unless from_account.objectable.is_teems_user?(to_account.id)
    end
  end
  ...
  def transfer
    ActiveRecord::Base.transaction do
      if valid?
        ...
        from_account.update!(balans: from_account.balans - amount)
        to_account.update!(balans: from_account.balans + amount)
        ActiveRecord::Base.transaction do
          save
        end
      end
